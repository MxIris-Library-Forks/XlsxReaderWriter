...
- (void)refreshContent {
    ...
    if (dictionaryRepresentation[@"r"]) {
        ...
        for (NSDictionary *textDict in runs) {
            NSAttributedString *attributedSubstring = [[NSAttributedString alloc] initWithString:[self stringFromTextDictionary:textDict] attributes:[self.styles attributedStringAttributesFromOpenXmlAttributes:textDict[@"rPr"]]];
    ...
    [_attributedString enumerateAttributesInRange:NSMakeRange(0, _attributedString.length)
                                         options:0 usingBlock:^(id value, NSRange range, BOOL *stop) {
                                             ...
                                             if (value[NSForegroundColorAttributeName]) {
                                                 ...
                                                 [runPropertiesDictionary setValue:[self.styles openXmlAttributesWithColor:color] forKeyPath:@"color"];
                                             }
