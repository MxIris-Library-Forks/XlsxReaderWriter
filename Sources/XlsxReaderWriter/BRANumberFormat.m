...
static NSArray<NSString *> *_availableCurrencySymbols;

@implementation NSArray (NSStringAdditions)

- (nullable NSString *)prefixOfString:(NSString *)string {
    for (id object in self) {
        if ([object isKindOfClass:[NSString class]]) {
            NSString *prefix = object;
...
+ (NSArray<NSString *> *)availableCurrencySymbols {
    if (!_availableCurrencySymbols) {
        _availableCurrencySymbols = @[
            @"¥",
            @"$",
            @"US$",
            @"€",
    ...
    return _availableCurrencySymbols;
}

- (instancetype)initWithFormatCode:(NSString *)formatCode andId:(NSInteger)formatId {
    NSDictionary *attributes = @{
        @"_formatCode": formatCode,
        @"_numFmtId": [NSString stringWithFormat:@"%ld", (long)formatId]
    };
    if (self = [super initWithOpenXmlAttributes:attributes]) {
...
//        _formatCode = GENERAL_NUMBER_FORMAT_CODE;
    ...
    NSMutableArray<BRANumberFormatData *> *numberFormats = @[].mutableCopy;
...
// TODO : [S] [M] [H]
            ...
            if ([_cacheData.code containsString:@"\\("] && [_cacheData.code containsString:@"\\)"]) {
                if ([_cacheData.color isEqual:BRANativeColor.redColor]) {
                    _cacheData.negativeStyle = BRANegativeStyleRedWithoutMinusSignInBraces;
                } else {
                    _cacheData.negativeStyle = BRANegativeStyleBlackWithoutMinusSignInBraces;
                }
            } else if ([_cacheData.code hasPrefix:@"-"] || [_cacheData.code hasPrefix:@"\\-"]) {
                if ([_cacheData.color isEqual:BRANativeColor.redColor]) {
                    _cacheData.negativeStyle = BRANegativeStyleRedWithMinusSign;
                } else {
                    _cacheData.negativeStyle = BRANegativeStyleBlackWithMinusSign;
                }
            } else if ([_cacheData.color isEqual:BRANativeColor.redColor]) {
                _cacheData.negativeStyle = BRANegativeStyleRedWithoutMinusSign;
            }
            BOOL isAccountingStyle = [_cacheData.code containsString:@"* "];
            ...
            if (_cacheData.currency) {
                _cacheData.type = kBRAFormatCodeTypeCurrency;
            }
            
            if (isAccountingStyle) {
                _cacheData.type = kBRAFormatCodeTypeAccounting;
            }
...
//    }
    ...
    if ([formatCode hasPrefix:@"[RED]"]) {
        return @"[RED]";
    } else if ([formatCode hasPrefix:@"[Red]"]) {
        return @"[Red]";
    }
...
- (BRANativeColor *)colorFromColorString:(NSString *)colorString {
    ...
    if ([colorString isEqual:@"[BLACK]"]) {
        ...
        return self.styles.indexedColors[colorIndex];
    }*/
...
- (NSString *)currencyFromCode:(NSString *)formatCode {
    ...
    NSArray<NSTextCheckingResult *> *results = [regEx matchesInString:formatCode options:0 range:NSMakeRange(0., formatCode.length)];
    ...
    if (results.count > 0 && [results[0] numberOfRanges] > 1) {
        ...
        if (currencyCode.length > 0) {
            ...
            result = [formatter currencySymbol];
        }
        
//        result = [formatCode substringWithRange:[results[0] range]];
    } else if ((result = [self.class.availableCurrencySymbols prefixOfString:formatCode])) {
...
- (NSAttributedString *)formatNumber:(CGFloat)number {
    ...
    if ([_formatCode isEqual:@"@"]) {
        ...
        stringToFormat = [NSString stringWithFormat:@"%@", @(number)];
