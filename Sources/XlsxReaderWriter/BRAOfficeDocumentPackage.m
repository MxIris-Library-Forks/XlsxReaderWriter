 ...
 */
+ (instancetype)open:(NSString *)filePath error:(NSError **)error {
    BRAOfficeDocumentPackage *document = [[self alloc] initWithContentsOfFile:filePath error:error];
    return document;
}
...
- (instancetype)initWithContentsOfFile:(NSString *)filePath error:(NSError **)error {
    ...
    if (self = [super init]) {
        ...
        BOOL isSuccess = [SSZipArchive unzipFileAtPath:filePath toDestination:self.cacheDirectory overwrite:YES password:nil error:error];
